...
exports.details = async (req, res) => {
    ...
    try {
        ...
        let productDetail = await ProductModel.findOne(filter, projection);
        if (!productDetail) {
...
exports.search = async (req, res) => {
    ...
    let Data = await ProductModel.find(findCriteria, projection, pageParams).sort(sort)
        .catch(err => {
            return {
                success: 0,
                message: 'Something went wrong while checking phone',
...
exports.reviews = async (req, res) => {
    ...
    try {
        ...
        let itemsCount = await Reviews.countDocuments({status: 1});
